...
class HealthMonitoringServer:
            ...
            if key in vitals and vitals[key] is not None:
                try:
                    ...
                    if value < min_val or value > max_val:
                        errors.append("{} value {} is outside valid range ({}-{})".format(
                            key, value, min_val, max_val))
                except ValueError:
                    errors.append("Invalid {} value: {}".format(key, vitals[key]))
        return errors
    ...
    def check_health_state(self, vitals):
        ...
        try:
            ...
            if errors:
                self.logger.warning("Health state check found issues: {}".format(errors))
                return "in danger"
            ...
            self.logger.error("Error checking health state: {}".format(e))
            return "unknown"
        ...
        def get_patients():
            try:
                with self.get_db_connection() as conn:
                    ...
                    self.logger.info("Retrieved {} patients".format(len(patients)))
                    return jsonify(patients)
            except Exception as e:
                self.logger.error("Error getting patients: {}".format(e))
                ...
                if request.method == 'GET':
                    ...
                    if len(data) == 1 and 'is_dispensing' in data:
                        with self.get_db_connection() as conn:
                            ...
                            if not c.fetchone():
                                return jsonify({'error': 'Patient {} not found'.format(patient_id)}), 404
                            ...
                            self.logger.info("Updated dispensing state for patient {}: {}".format(
                                patient_id, is_dispensing))
                            return jsonify({
                                'status': 'success',
                                'message': 'Updated dispensing state for patient {}'.format(patient_id),
                    ...
                    for field, field_type in required_fields.items():
                        ...
                        try:
                            ...
                            return jsonify({'error': 'Invalid type for field {}'.format(field)}), 400
                    ...
                    with self.get_db_connection() as conn:
                        ...
                        self.logger.info("Updated patient {} data successfully".format(patient_id))
                        return jsonify({'status': 'success'})

            except sqlite3.IntegrityError as e:
                self.logger.error("Database integrity error: {}".format(e))
                return jsonify({'error': 'Database integrity error', 'details': str(e)}), 400
            except Exception as e:
                self.logger.error("Error updating patient: {}".format(e))
                return jsonify({'error': str(e)}), 500
            ...
            try:
                ...
                with self.get_db_connection() as conn:
                    ...
                    query = '''UPDATE patients SET 
                              {}, timestamp = ?
                              WHERE id = ?'''.format(', '.join(update_fields))
                    
                    c.execute(query, params)
                    return jsonify({'status': 'success'})
                    
            except Exception as e:
                self.logger.error("Vitals update error: {}".format(e))
                return jsonify({'error': str(e)}), 500
    ...
    def run(self):
        ...
        try:
            ...
            self.logger.error("Server failed to start: {}".format(e))
            raise
